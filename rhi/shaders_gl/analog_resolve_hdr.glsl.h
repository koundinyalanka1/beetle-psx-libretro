...
#ifndef GL_ANALOG_RESOLVE_HDR_GLSL_H
#define GL_ANALOG_RESOLVE_HDR_GLSL_H

/* Push-constant members, as GL uniform names:
 *   vec2     reg_sig_size
 *   vec2     reg_out_size
 *   int      reg_sdr_eotf
 *   float    reg_paper_white_nits
 *   float    reg_peak_nits
 *   int      reg_expand_gamut
 *   int      reg_shoulder
 *   int      reg_src_primaries
 */

static const char *analog_resolve_hdr_glsl =
   "#version 330 core\n"
   "#define HDR 1\n"
   "\n"
   ...
   "\n"
   "uniform sampler2D uDecoded;\n"
   "\n"
   "uniform vec2 reg_sig_size;\n"
   "uniform vec2 reg_out_size;\n"
   "uniform int reg_sdr_eotf;\n"
   "uniform float reg_paper_white_nits;\n"
   "uniform float reg_peak_nits;\n"
   "uniform int reg_expand_gamut;\n"
   "uniform int reg_shoulder;\n"
   "uniform int reg_src_primaries;\n"
   "\n"
   "void main()\n"
   "{\n"
   ...
   "	highp float bpo = reg_sig_size.x / max(reg_out_size.x, 1.0);\n"
   ...
   "		if (i >= num)\n"
   "			break;\n"
   "		highp float k = clamp(n0 + float(i), 0.0, reg_sig_size.x - 1.0);\n"
   "		highp vec3  v = textureLod(uDecoded, vec2((k + 0.5) / reg_sig_size.x, row), 0.0).rgb;\n"
   "		acc += sdr_to_linear(clamp(v, vec3(0.0), vec3(1.0)), reg_sdr_eotf);\n"
   "		cnt += 1.0;\n"
   "	}\n"
   "\n"
   ...
   "	FragColor = vec4(encode_hdr10_linear(lin, reg_paper_white_nits, reg_peak_nits,\n"
   "	                                     reg_expand_gamut, reg_shoulder,\n"
   "	                                     reg_src_primaries), 1.0);\n"
   "#else\n"
   ...
   "	FragColor = vec4(linear_to_sdr(src_primaries_to_709(lin, reg_src_primaries),\n"
   "	                               reg_sdr_eotf), 1.0);\n"
   "#endif\n"
   "}\n"
   ;

static const char *analog_resolve_hdr_glsl_ES =
   "#version 300 es\n"
   "precision highp float;\n"
   "precision highp int;\n"
   "#define HDR 1\n"
...
#endif /* GL_ANALOG_RESOLVE_HDR_GLSL_H */
