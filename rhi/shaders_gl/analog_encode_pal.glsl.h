...
#ifndef GL_ANALOG_ENCODE_PAL_GLSL_H
#define GL_ANALOG_ENCODE_PAL_GLSL_H
...
static const char *analog_encode_pal_glsl =
   "#version 330 core\n"
   "#define PAL 1\n"
   "\n"
   ...
   "uniform float reg_div;\n"
   "uniform float reg_x1;\n"
   "uniform float reg_inv_ratio;\n"
   "uniform float reg_line_adv;\n"
   "uniform float reg_line_split;\n"
   "uniform float reg_field_off;\n"
   "uniform float reg_field_adv;\n"
   "uniform int reg_cable;\n"
   "\n"
   ...
   "	if (reg_cable == AN_CABLE_SVIDEO)\n"
   "		return AN_LUMA_WIDE_P[b] - AN_LUMA_WIDE_P[a];\n"
   "	if (reg_cable == AN_CABLE_RF)\n"
   ...
   "\n"
   "	int   idiv = int(reg_div);\n"
   "	int   ni   = int(n);\n"
   "	int   maxp = int(reg_src_size.x) - 1;\n"
   "\n"
   ...
   "			textureLod(uSource, vec2((px + 0.5) / reg_src_size.x, row), 0.0).rgb);\n"
   "\n"
   "		y  += wl * yc.x;\n"
   "		iq += wc * yc.yz;\n"
   "	}\n"
   "\n"
   "	highp float ph = an_phase(n, line, reg_x1, reg_inv_ratio,\n"
   "	                          reg_line_adv, reg_line_split, reg_field_off,\n"
   "	                          reg_field_adv);\n"
   "	highp vec2  cs = an_carrier(ph);\n"
   "	highp float vs = an_v_sign(an_field_line(line, reg_line_split));\n"
   "\n"
...
static const char *analog_encode_pal_glsl_ES =
   "#version 300 es\n"
   "precision highp float;\n"
   "precision highp int;\n"
   "#define PAL 1\n"
...
#endif /* GL_ANALOG_ENCODE_PAL_GLSL_H */
