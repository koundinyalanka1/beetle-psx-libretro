...
struct _flac_decoder {
	...
	int16_t                 sector_buf[2352];		/* one CD sector of decoded samples; in the
													 * struct rather than decode_interleaved's
													 * frame, which has to fit an 8 KiB thread
													 * stack on some targets */
};
